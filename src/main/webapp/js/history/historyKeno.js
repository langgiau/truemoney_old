...
function historyInit() {
        ...
        if (($(window).scrollTop() + $(window).height()) >= ($(document).height() - 100)) {
            if (historyPageLoadingKey) {
                ...
                if (commonIsEmpty(urlType) != "") {
                    if (urlType == 1) {
                        historyGetDataBuyDone("#historyBodyKenoBuyDone");
                    } else if (urlType == 2) {
                        historyGetDataBuyCancel("#historyBodyKenoBuyCancel");
                    }
                }
            }
        }
    ...
    var urlType = commonGetParameterByName("type");
    if (commonIsEmpty(urlType) != "") {
        var tabSelected = "historyBodyKenoWaiting";
        if (urlType == 0) {
            tabSelected = "historyBodyKenoWaiting";
        } else if (urlType == 1) {
            tabSelected = "historyBodyKenoBuyDone";
        } else if (urlType == 2) {
            tabSelected = "historyBodyKenoBuyCancel";
        }
...
function historyGetDataWaiting(idTab) {
    var urlInfo = "/action/history/getHistoryKenoWaiting";
    var obj = {        
...
function historyGetDataBuyDone(idTab) {
    var urlInfo = "/action/history/getHistoryKenoTransDone";
    var obj = {
        page_index: historyCountPageIndex + "",
        optStatus: "3"
    };
    ...
    //    console.log("adbc:" +result);
        
        if (result != null) {
            dataAll = result.data;
        //    console.log("abc:" + dataAll);
            
            if (dataAll != null && dataAll.length > 0) {
                historyCountPageIndex = historyCountPageIndex + 1;
            //    console.log("page:"+historyCountPageIndex);
            }
            historyGroupTimeDateDataAll(idTab, dataAll);
        }
    });
...
function historyOrderInfoStatus(key) {
    ...
    if (key == 0) {
        ...
        html = '<i class="fa fa-check-circle-o" style="color: #59c406"><span style="font-family: Roboto;font-size: small;"> Đã bị hủy bởi khách hàng</span></i>';
    }
...
function historyOpenLinkDetailById(orderId,orderIdMask) {
    window.location.href = requestUrl + "/history/detail?id=" + orderId + "&type=" + historyTabEventClick +"&idMask="+orderIdMask +"&keno="+1;
}
